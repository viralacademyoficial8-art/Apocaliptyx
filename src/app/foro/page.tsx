...
function ForoContent() {
  ...
  const handleActivityLike = async (activityId: string) => {
    if (!user?.id) {
      router.push('/login');
      return;
    }
  ...
  const handleActivityBookmark = async (activityId: string) => {
    if (!user?.id) {
      router.push('/login');
      return;
    }
                    ...
                    if (item.type === 'post') {
                      ...
                      return (
                        <ActivityCard
                          ...
                          onBookmark={handleActivityBookmark}
            ...
            <div className="p-3 sm:p-4 border-t border-border flex-shrink-0 bg-card">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-600/50 disabled:cursor-not-allowed rounded-lg text-white font-medium transition-colors flex items-center"
                >
...
function ActivityCard({ item, dateLocale, onLike, onBookmark, onOpenComments }: ActivityCardProps) {
  const router = useRouter();
  ...
  const shareToTwitter = () => {
    const text = `${item.user.displayName || item.user.username}: ${getPostContent().slice(0, 100)}`;
    const url = item.metadata?.scenarioId
      ? `${window.location.origin}/escenario/${item.metadata.scenarioId}`
      : window.location.href;
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`, '_blank');
    ...
    <div className="bg-card border border-border rounded-xl p-5 hover:border-purple-500/30 transition-all shadow-sm">
      ...
      <div className="flex items-center gap-2 text-muted-foreground mb-3">
        ...
        {scenarioLink && (
          <Link
            href={scenarioLink}
            className="ml-auto flex items-center gap-1.5 px-3 py-1.5 bg-purple-500 text-white rounded-lg text-sm font-medium hover:bg-purple-600 transition-colors"
          >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border">
        ...
        >
          <Heart className={`w-5 h-5 ${item.user_liked ? 'fill-current' : ''}`} />
        ...
        <div className="relative">
          ...
          {showShareMenu && (
            <div className="absolute bottom-full right-0 mb-2 bg-muted border border-border rounded-lg shadow-xl z-50 min-w-[160px] py-2">
              <button
