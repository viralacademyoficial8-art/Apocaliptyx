...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get user's joined communities first
    ...
    // Get ALL communities (both public and private)
    // Private communities will appear locked for non-members
    let query = supabase()
      .from('communities')
      .select('*')
    ...
    let allCommunities = (allCommunitiesRaw as CommunityRow[] | null) || [];

    // Filter if needed
    let filteredCommunities = allCommunities;
    if (filter === 'joined' && session?.user?.id) {
      // For "Mis comunidades", only show communities user is a member of
